...
for Indx_P = 1:numel(Participants)
    for Indx_S = 1:numel(Sessions)
        for Indx_T = 1:numel(Tasks)
            ...
            Keep_Fields  = {'nPeaks', 'amplitude', 'period', 'Band', ...
                'Channel', 'Sign', 'Start', 'End', 'Frequency', 'Coh_Burst_Channels', ...
                'Coh_Burst_Starts', 'Coh_Burst_Ends', 'Coh_Burst_nPeaks', 'Coh_amplitude', ...
                'All_Start', 'All_End', 'globality_bursts', 'Mean_amplitude', ...
                'Mean_period', 'Type'};

            RM_Fields = fieldnames(Bursts);
            RM_Fields(ismember(RM_Fields, Keep_Fields)) = [];
            Bursts = rmfield(Bursts, RM_Fields);
