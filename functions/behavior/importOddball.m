function AllAnswers = importOddball(DatasetPaths, Task, Destination)
% TEMP: old function to load in task data, adapted to the oddball
...
% find all paths
[Subfolders, Datasets] = AllFolderPaths(DatasetPaths, 'PXX', false, {'CSVs', 'Lazy', 'P00', 'Applicants', 'Uncertain'});
...
for Indx_P = 1:size(Datasets, 1)
    for Indx_S = 1:numel(Subfolders)
        ...
        extraFields = {'Participant', 'Task', 'Session', 'Filename', 'ALARM_';
            Participant, Task, Session{1}, Files{1}, nan};
        
        Output = importOddballOutput(fullfile(Folder, Files{1}), 'table', extraFields);
